...
pub fn init(b: *std.Build, appVersion: []const u8) !Config {
    ...
    config.@"enable-browser" = b.option(
        bool,
        "enable-browser",
        "Enable embedded browser pane support. On macOS uses system WebKit.",
    ) orelse false;
...
pub fn addOptions(self: *const Config, step: *std.Build.Step.Options) !void {
    ...
    step.addOption(bool, "enable-browser", self.@"enable-browser");
...
pub fn fromOptions() Config {
    ...
    return .{
        ...
        .i18n = options.i18n,
        .@"enable-browser" = options.@"enable-browser",
    };
}
