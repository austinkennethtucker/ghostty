...
const framedata = @import("trident_framedata").compressed;
...
const TridentAnim = struct {
    ...
    fn widget(self: *TridentAnim) vxfw.Widget {
        return .{
            .userdata = self,
            .eventHandler = TridentAnim.typeErasedEventHandler,
            .drawFn = TridentAnim.typeErasedDrawFn,
        };
    }

    fn typeErasedEventHandler(ptr: *anyopaque, ctx: *vxfw.EventContext, event: vxfw.Event) anyerror!void {
        const self: *TridentAnim = @ptrCast(@alignCast(ptr));
        switch (event) {
            .init,
            .tick,
            => {
    ...
    fn updateFrame(self: *TridentAnim) void {
        ...
        // outline style until the next '<', and skip until the next '>'
...
/// The `trident` command displays the Trident animation in the terminal
