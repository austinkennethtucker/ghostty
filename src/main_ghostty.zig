...
pub fn main() !MainReturn {
    ...
    if (comptime builtin.mode == .Debug) {
        ...
        std.log.warn("You should only use a debug build for developing Trident.", .{});
    ...
    if (comptime build_config.app_runtime == .none) {
        ...
        try stdout.print(
            \\This is the Trident helper CLI that accompanies the graphical Trident app.
            \\To launch the terminal directly, please launch the graphical app
            \\(i.e. Trident.app on macOS). This CLI can be used to perform various
            \\actions such as inspecting the version, listing fonts, etc.
            \\
            \\On macOS, the terminal can also be launched using `open -na Trident.app`,
            \\or `open -na Trident.app --args --foo=bar --baz=qux` to pass arguments.
            \\
