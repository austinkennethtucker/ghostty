...
/// Set Trident's graphical user interface language to a language other than the
/// system default language. For example:
///
///     language = de
///
/// will force the strings in Trident's graphical user interface to be in German
/// rather than the system default.
///
/// This will not affect the language used by programs run _within_ Trident.
/// Those will continue to use the default system language. There are also many
/// non-GUI elements in Trident that are not translated - this setting will have
/// no effect on those.
///
/// Warning: This setting cannot be reloaded at runtime. To change the language
/// you must fully restart Trident.
///
/// GTK only.
...
/// etc.
///
/// Notes on emoji specifically: On macOS, Trident by default will always use
...
/// found, Trident will use the regular style. This prevents falling back to a
...
/// set `font-family-bold = FooBar` and "FooBar" cannot be found, Trident will
/// use whatever font is set for `font-family` for the bold style.
///
/// Finally, some styles may be synthesized if they are not supported.
/// For example, if a font does not have an italic style and no alternative
/// italic font is specified, Trident will synthesize an italic style by
...
/// Control whether Trident should synthesize a style if the requested style is
/// not available in the specified font-family.
///
/// Trident can synthesize bold, italic, and bold italic styles if the font
...
///
/// The font feature will apply to all fonts rendered by Trident. A future
...
///
/// Trident breaks runs at certain points to improve readability or usability.
/// For example, Trident by default will break runs under the cursor so that
...
///
/// This configuration only applies to Trident builds that use FreeType.
...
///     This is the most common setting in GTK apps and therefore also Trident's
...
/// name, or an absolute path to a custom theme file. Trident also supports
/// specifying a different theme to use for light and dark mode. Each
/// option is documented below.
///
/// If the theme is an absolute pathname, Trident will attempt to load that
...
///
/// The first directory is the `themes` subdirectory of your Trident
/// configuration directory. This is `$XDG_CONFIG_HOME/ghostty/themes` or
/// `~/.config/ghostty/themes`.
///
/// The second directory is the `themes` subdirectory of the Trident resources
/// directory. Trident ships with a multitude of themes that will be installed
/// into this directory. On macOS, this list is in the
/// `Trident.app/Contents/Resources/ghostty/themes` directory. On Linux, this
/// list is in the `share/ghostty/themes` directory (wherever you installed the
/// Trident "share" directory.
///
/// To see a list of available themes, run `ghostty +list-themes`.
///
/// A theme file is simply another Trident configuration file. They share
...
/// `config-file`. At the time of writing this, Trident will not show any
...
/// splits. A future improvement to Trident will address this.
///
...
/// memory usage (specifically VRAM usage). A future Trident improvement
...
///
/// If this is not set at all (`null`), then Trident will respect DEC Mode 12
...
/// Trident's shell integration features to be enabled.
///
...
///   anything but modifiers or keybinds that are processed by Trident).
///
...
/// to all scrolling devices. Specifying a prefix was introduced in Trident
/// 1.2.1.
///
...
///
/// On macOS, changing this configuration requires restarting Trident completely.
...
/// created when Trident starts, use the `initial-command` configuration.
///
/// Trident supports the common `-e` flag for executing a command with
...
/// surface created when Trident starts. Subsequent terminal surfaces will use
...
///
///   * `quit-after-last-window-closed=true` - This ensures that the Trident
...
/// variables set by Trident. For example, if you set `GHOSTTY_RESOURCES_DIR`
/// then the value you set here will override the value Trident typically
/// automatically injects.
///
/// These environment variables _will not_ be passed to commands run by Trident
...
/// The title Trident will use for the window. This will force the title of the
/// window to be this title at all times and Trident will ignore any set title
...
/// bus name that Trident uses to connect to DBus.
///
/// Note that changing this value between invocations will create new, separate
/// instances, of Trident when running with `gtk-single-instance=true`. See that
/// option for more details.
///
/// Changing this value may break launching Trident from `.desktop` files, via
/// DBus activation, or systemd user services as the system is expecting Trident
...
/// setting. If a previous Trident terminal exists in the same process,
...
/// if Trident can detect it is launched from launchd (double-clicked) or
/// `open`, then it defaults to `home`. On Linux with GTK, if Trident can detect
...
/// otherwise bound. When looking up a binding, Trident first tries to match
...
///
///   * Trident will wait an indefinite amount of time for the next key in
...
///  * `global:`
///
///    Make the keybind global. By default, keybinds only work within Trident
...
///    (e.g. even when Trident is not focused), specify this prefix.
...
///    if there is a selection to copy. If there is no selection, Trident
...
/// to Trident. When a `global:` keybind is specified and Trident is launched
/// or reloaded, Trident will attempt to request these permissions.
...
/// sequence.
///
/// Chained keybinds are available since Trident 1.3.0.
///
/// ## Key Tables
///
...
///
/// Key tables are available since Trident 1.3.0.
keybind: Keybinds = .{},

/// Remap modifier keys within Trident. This allows you to swap or reassign
...
///
/// This will NOT change keyboard layout or key encodings outside of Trident.
...
/// before Trident receives the key events.
///
/// Example:
///
...
///   prior to Trident receiving them. For example, `option+a` on macOS
...
///   this happens before Trident receives the key event. To workaround
...
/// may override this preference. Trident will do its best to respect this
...
///    desktop environment. This option usually makes Trident look the most
...
/// certain size to fit within the grid. There is nothing Trident will do about
...
/// to be a certain position to fit within the grid. There is nothing Trident
...
///
///   * `always` will always save window state whenever Trident is exited.
///
/// If you change this value to `never` while Trident is not running, the next
/// Trident launch will NOT restore the window state.
///
/// If you change this value to `default` while Trident is not running and the
/// previous exit saved state, the next Trident launch will still restore the
/// window state. This is because Trident cannot know if the previous exit was
/// due to a forced save or not (macOS doesn't provide this information).
///
/// If you change this value so that window state is saved while Trident is not
/// running, the previous window state will not be restored because Trident only
...
/// This is targeted directly at using Trident from the CLI in a way
...
///
/// On Linux, if this is `true`, Trident can delay quitting fully until a
/// configurable amount of time has passed after the last window is closed.
/// See the documentation of `quit-after-last-window-closed-delay`.
@"quit-after-last-window-closed": bool = builtin.os.tag == .linux,

/// Controls how long Trident will stay running after the last open surface has
...
/// Trident will quit immediately after the last window is closed if
/// `quit-after-last-window-closed` is `true`.
///
/// Only implemented on Linux.
@"quit-after-last-window-closed-delay": ?Duration = null,

/// This controls whether an initial window is created when Trident
/// is run. Note that if `quit-after-last-window-closed` is `true` and
/// `quit-after-last-window-closed-delay` is set, setting `initial-window` to
/// `false` will mean that Trident will quit after the configured delay if no
...
///
/// On macOS, changing this configuration requires restarting Trident
/// completely.
///
...
///     Attempts to install Trident's terminfo entry using `infocmp` and `tic` when
...
///     (Available since: 1.2.0)
///
///   * `path` - Add Trident's binary directory to PATH. This ensures the `ghostty`
...
/// when both `ssh-env` and `ssh-terminfo` are enabled, Trident will install its
...
/// command-palette-entry = title:"Trident",description:"Add a little Trident to your terminal.",action:"text:\xf0\x9f\x91\xbb"
/// ```
///
...
/// Trident currently supports OSC 10 (foreground), OSC 11 (background), and
...
///
/// Warning: Invalid shaders can cause Trident to become unusable such as by
...
/// largely match Shadertoy, with some caveats and Trident-specific extensions.
///
...
///
///    Request the user's attention when Trident is unfocused, until it has
...
///      bring the Trident window into focus;
///
...
/// Control the in-app notifications that Trident shows.
///
...
/// and not something Trident enables.
///
...
/// onto the Trident icon in the macOS dock.
///
/// Valid values are:
///
...
/// If true, Trident on macOS will automatically enable the "Secure Input"
/// feature when it detects that a password prompt is being displayed.
///
/// "Secure Input" is a macOS security feature that prevents applications from
/// reading keyboard events. This can always be enabled manually using the
/// `Trident > Secure Keyboard Entry` menu item.
///
...
/// If true, Trident will show a graphical indication when secure input is
...
/// If true, Trident exposes and handles the built-in AppleScript dictionary
/// on macOS.
///
...
/// Valid values:
///
///  * `official` - Use the official Trident icon.
///  * `blueprint`, `chalkboard`, `microchip`, `glass`, `holographic`,
///    `paper`, `retro`, `xray` - Official variants of the Trident icon
...
///  * `custom-style` - Use the official Trident icon but with custom
...
/// Whether macOS Shortcuts are allowed to control Trident.
///
/// Trident exposes a number of actions that allow Shortcuts to
/// control and interact with Trident. This includes creating new
...
/// Valid values are:
///
/// * `ask` - Ask the user whether for permission. Trident will remember
///   this choice and never ask again. This is similar to other macOS
///   permissions such as microphone access, camera access, etc.
///
/// * `allow` - Allow Shortcuts to control Trident without asking.
///
/// * `deny` - Deny Shortcuts from controlling Trident.
///
/// Available since: 1.2.0
...
/// instead of the entire Trident process. Similarly, if a shell program is
/// using too much CPU, only that surface will be CPU-throttled.
///
/// This will cause startup times to be slower (a hundred milliseconds or so),
/// so the default value is "single-instance." In single-instance mode, only
/// one instance of Trident is running (see gtk-single-instance) so the startup
/// time is a one-time cost. Additionally, single instance Trident is much
/// more likely to have many windows, tabs, etc. so cgroup isolation is a
/// big benefit.
///
/// This feature requires `systemd`. If `systemd` is unavailable, cgroup
/// initialization will fail. By default, this will not prevent Trident from
...
///   * `single-instance` - Enable cgroups only for Trident instances launched
...
/// If `true`, the Trident GTK application will run in single-instance mode:
/// each new `ghostty` process launched will result in a new window if there is
/// already a running process.
///
/// If `false`, each new ghostty process will launch a separate application.
///
/// If `detect`, Trident will assume true (single instance) unless one of
/// the following scenarios is found:
///
/// 1. TERM_PROGRAM environment variable is a non-empty value. In this
/// case, we assume Trident is being launched from a graphical terminal
/// session and you want a dedicated instance.
///
/// 2. Any CLI arguments exist. In this case, we assume you are passing
/// custom Trident configuration. Single instance mode inherits the
...
/// The default value is `detect`.
///
/// Note that debug builds of Trident have a separate single-instance ID
...
/// If `true` (default), then the Trident GTK tabs will be "wide." Wide tabs
...
///
/// Launch Trident with `env GTK_DEBUG=interactive ghostty` to tweak Trident's
/// CSS in real time using the GTK Inspector. Errors in your CSS files would
/// also be reported in the terminal you started Trident from. See
...
/// escape sequences such as OSC 4. (Since Trident 1.2.0, the previous
/// configuration `bold-is-bright` is deprecated and replaced by this
/// usage).
///
/// Available since Trident 1.2.0.
...
///
/// If the selected backend is not available on the platform, Trident will
...
/// Control the auto-update functionality of Trident. This is only supported
/// on macOS currently, since Linux builds are distributed via package
/// managers that are not centrally controlled by Trident.
///
...
/// underlying protocols. To put it another way: Trident doesn't explicitly
...
/// running Trident version. If you download a pre-release version of Trident
/// then this will be set to `tip` and you will receive pre-release updates.
/// If you download a stable version of Trident then this will be set to
...
/// Set to true if Trident was executed as xdg-terminal-exec on Linux.
...
/// Trident in a CLI environment. We need this to change some behaviors.
...
/// See macos-icon
///
/// Note: future versions of Trident can support a custom icon with
